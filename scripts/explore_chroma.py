...
<body>
    ...
    <div id="collections">
        ...
        <div class="collection" id="collection-{{ collection.name }}">
            <h2>
                <span class="collection-name">{{ collection.name }}</span>
                <span class="count">({{ collection.count }} documents)</span>
                <button class="delete-btn" onclick="deleteCollection('{{ collection.name }}')">Delete Collection</button>
            </h2>
    ...
    function deleteCollection(collectionName) {
        if (confirm('Are you sure you want to delete this collection?')) {
            ...
            .then(data => {
                if (data.success) {
                    document.getElementById('collection-' + collectionName).remove();
                } else {
